...
from constants import BODY, TWEET_MAPPINGS
...
class CSVRowFormatter:
    def __init__(self, fieldnames: list) -> None:
        ...
        self.boolean_fields = [
            k for k, v in TWEET_MAPPINGS["properties"].items() if v["type"] == "boolean"
        ]
        self.integer_fields = [
            k for k, v in TWEET_MAPPINGS["properties"].items() if v["type"] == "integer"
        ]
        self.float_fields = [
            k for k, v in TWEET_MAPPINGS["properties"].items() if v["type"] == "float"
        ]
...
def scroll_index(
    client: OpenSearch, index: str, pagesize=250, scroll_timeout="1m", **kwargs
):
    is_first = True
    scroll_id = None
    while True:
